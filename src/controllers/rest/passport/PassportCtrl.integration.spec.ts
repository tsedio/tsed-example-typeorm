...
describe("PassportCtrl", () => {
  ...
  describe("POST /rest/auth/login", () => {
    ...
    it("should throw an error if user doesn't exists", async () => {
      ...
      expect(response.body).toEqual({name: "AuthenticationError", message: "Unauthorized", status: 401, errors: []});
    });
    ...
    it("should throw an error if the password is missing", async () => {
      ...
      expect(response.body).toEqual({name: "AuthenticationError", message: "Bad Request", status: 400, errors: []});
    });
